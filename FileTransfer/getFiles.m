...
if ~isempty(fileName)
    command = [sprintf('RSYNC_PASSWORD=alpine rsync -Pav rsync://root@localhost:%d', telnetPort), '/root/var/root/', fileName, ' ', localDirectoryPath];
    [status,cmdout] = system(command);
elseif ~isempty(fileExtension)
    command = [sprintf('RSYNC_PASSWORD=alpine rsync -Pav rsync://root@localhost:%d', telnetPort), '/root/var/root/*', fileExtension, ' ', localDirectoryPath];
    [status,cmdout] = system(command);
else
